...
## Tipos de Datos en R
- Todos los objetos en R están construidos a partir de estos tres tipo de *objetos atómicos
...
mean(x, na.rm=FALSE)
```
- Las unciones tiene argumentos con nombre (*tagged*), por ejemplo `na.rm` que asmen valores por defecto 
- Los argumentos sin nombre se valores posicionales (*positional*) y por lo general represetan el vector de datos al que se dea aplicar la función.
...
- Las unciones tiene argumentos con nombre (*tagged*), por ejemplo `na.rm` que asmen valores por defecto 
- Los argumentos sin nombre se valores posicionales (*positional*) y por lo general represetan el vector de datos al que se dea aplicar la función.
- Los operadores básicos son funciones, por ejeplo `x + y` puede ser ejcutado como `'+'(x,y)`
- Los operados básicos se pueden combinar con las funciones y se ejecuta como de manera semejante a la resolución matematica.
```{r, echo=TRUE, results="hide"}
x <- c(2,1,8,3)
y <- 2 * (0.5 + log(x))
```
...
- Los paquetes son la unidad fundamental del codigo compartido en R (Wickham, 2015)
- Los paquets contienen funciones que permiten apmpliar la capacidad de realizar operaciones con R
...
- Los paquets contienen funciones que permiten apmpliar la capacidad de realizar operaciones con R
- Hay mas de  paquetes en el Comprehensive R Archive Network o CRAN.
- Un paquete contiene las funciones, su documentación (archivos de ayuda) y las *vignettes* (guia de uso)
- Es necesario instalar los paquetes para usarlos.

## Usando los paquetes en R
- Para instalr un paquete se usa el siguiente código
...
## Usando los paquetes en R
- Existen paquetes que están en estado de desarrollo y se encuentran disponibles en repositorios publicos como [GitHub](https://github.com/)
\begin{center}
...
# Impportando y exportando datos

## Importando y exportando información
- El proceso de importar y exporatar información depende del formato en que la información
- Para importa y exportar es necesario entregar la informacion del *path* o configurar el directorio de trebajo
```{r, echo=TRUE,eval=FALSE}
setwd("path")
getwd()
```
...
```
- Para otrso tipos de archivos de texto plano pueden usarse las funcones `read.table()` o `read.delim()`
...
```
- Es posible realizar subconjuntos de datos qeu cumplan un conjunto de condiciones mediante la función `subset`
```{r, echo=TRUE,results="hide"}
...
### ¿Cúantos autos tiene 4 cilindros?
```{r,echo=TRUE}
cyl4 <- mtcars[mtcars$cyl==4, ]
cyl4_1 <- subset(mtcars, cyl==4)
```

### ¿Cuántos autos tienen 4 cilindors y mas de 90 caballos de fuerza?
\pause
```{r,echo=TRUE}
...
### ¿Cúales celdas contienen datos con autos con 6 cilindros?
```{r,echo=TRUE}
which(mtcars$cyl==6)
```
